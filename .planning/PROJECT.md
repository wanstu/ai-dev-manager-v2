...
- Phase 23 detailed planning is ready. `ADM-CLI-001` keeps normal CLI automation on Admin MCP and scopes delivery to MCP file/stdin import + owner-runtime diagnostics, Skill source/availability parity, Phase-20 Environment context output and Phase-22 temporary Environment lifecycle. Feature implementation has not started.
- Transient navigation/filter/focus/lifecycle-observation state is presentation state; existing connection/autostart preferences remain the prior explicit exception. Phases 18-22 add no second state model or persistence path; Phase 23 planning likewise adds none.
...
Phase 23 detailed planning lives under `.planning/phases/23-cli-agent-ux/23-CONTEXT.md`, `23-01-PLAN.md` and `23-02-PLAN.md`. `ADM-CLI-001` keeps the normal CLI as a thin client over the same Admin MCP/application contracts: no direct writable state fallback, no hidden current Environment, no CLI-only authority or task semantics. 23-01 is ready and focuses on MCP import-from-file/stdin, passive owner-runtime MCP inspect vs explicit refresh, Skill source update/multiple support roots, and global/Environment Skill availability. 23-02 remains gated and will expose the existing Phase-20 `environment_context_bundle` and Phase-22 temporary Environment create/status/promote/preview/execute lifecycle through the same Admin MCP client, then close the phase with fixed-head CLI/full/vet evidence. Existing pretty JSON is already the normal management output baseline; Phase 23 does not add a universal formatter. There are no new prerequisites and no Desktop scope.
...
---
*Last updated: 2026-09-13 for Phase 23 detailed planning. The 2026-09-08 audit is historical; Phases 18-22 are complete, Phase 23 planning is ready, and feature implementation has not started.*
