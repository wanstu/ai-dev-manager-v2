...
- Phase 22-01 and 22-02 are complete and validated. Core supports atomic owner+TTL temporary Environment creation for ordinary existing roots and optional managed worktrees, owner-scoped retention-only promotion, one-Environment targeted non-force cleanup, and runtime cleanup blockers including active `vfrun_`; Gateway commit `d51a426` exposes the constrained lifecycle through shared Agent/Admin tools with real Streamable HTTP acceptance. 22-03 is ready and has not started.
- Transient navigation/filter/focus state is presentation state; existing connection/autostart preferences remain the prior explicit exception. Phases 18-21 add no second state model; Phase 22 planning likewise introduces no second persistence model.
...
## Active Planning — Phase 22

Phase 22 detailed planning lives under `.planning/phases/22-temporary-task-environments/22-CONTEXT.md`, `22-01-PLAN.md`, `22-02-PLAN.md` and `22-03-PLAN.md`. `ADM-CORE-022` keeps temporary task work as lifecycle infrastructure rather than an ADM task model. 22-01 and 22-02 are complete: temporary Environments are normal `env_` resources created atomically with explicit lifecycle owner + positive TTL, optional session/run provenance only, and either an existing Workspace-contained root or optional managed-worktree isolation. Targeted cleanup previews/rechecks one requested Environment only, preserves ordinary project directories, reuses non-force managed-worktree destroy safety, includes active `vfrun_` among runtime blockers, and never sweeps unrelated resources; matching-owner promotion changes retention only. Shared Agent/Admin `environment_temporary_create/status/promote/cleanup` tools are delivered at `d51a426` with real Streamable HTTP acceptance and no Agent exposure of generic durable `environment_create`. 22-03 is ready for bounded Desktop retention visibility/actions through Admin MCP and integrated closeout. CLI lifecycle UX remains Phase 23. There are no new global prerequisites.
...
---
*Last updated: 2026-09-13 for Phase 22-02 completion. The 2026-09-08 audit is historical; Phases 18-21 are complete, Phase 22-01/22-02 are complete, and 22-03 is ready but not started.*
